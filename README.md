...
Pressing the **Load** button calls the function *timerLoad*. First, a variable called *errorLoad* is created and is set to false, the variable is set to true whenever a wrong value is found by the function. Before the data can be read an if statement is checked for the variable *loadSkip*. If the variable is set to true that means that *saveTimers* already has the information it needs loaded so the code can skip looking at whats on the previous page. *loadSkip* is false by default and its set to true when all timers are finished and the looping is on or if a session is loaded from local memory. *timerLoad* then goes into every row, using a for loop, checking every numeric value and check state and storing it inside *saveTimers* with its corresponding ID number. If an incorrect value is found *addAlert* is called displaying an error message, deleting the corresponding value and setting *errorLoad* to true. If an error is found *errorLoad* is set to false and the function returns before finishing. 

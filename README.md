...
Used when the timers are running. Called by *btn-next*. Changes the current timer to the next one in the page. 

## padZero()

Called when a number is rendered on screen. Add a leading zero. 

## play()

Plays a sound when a timer is donde. Called by *downTimer()*

## renderTimers()

Takes the data in *saveTimers* and renders a session. 

## showMenu()

Toogles the visibility of the dropdown menu.

## timerLoad()

Loads the information added by the user to *saveTimers*.

## undoTimer(name)

Restores a deleted timer back into local memory.